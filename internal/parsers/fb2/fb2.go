...
func (fb *FB2) GetSort() string {
	return parsers.GetSortTitle(fb.Description.TitleInfo.BookTitle, parsers.GetLanguageTag(fb.Description.TitleInfo.Lang))
}
...
func (fb *FB2) GetPlot() string {
	return parsers.StripHTMLTags(strings.Join(fb.Description.TitleInfo.Annotation.P, " "))
}
...
func (fb *FB2) GetLanguage() *model.Language {
	return parsers.GetLanguage(fb.Description.TitleInfo.Lang)
}
...
func (fb *FB2) GetAuthors() []*model.Author {
	...
	if len(fb.Description.TitleInfo.Authors) == 1 &&
		...
		for _, a := range aLN {
			author := parsers.AuthorByFullName(a)
			if author.Sort != "" {
	...
	for _, a := range fb.Description.TitleInfo.Authors {
		author := parsers.AuthorByFullName(fmt.Sprintf("%s %s %s", a.FirstName, a.MiddleName, a.LastName))
		if author.Sort != "" {
			authors = append(authors, author)
		}
	}
...
func (fb *FB2) GetSerie() *model.Serie {
	if len(fb.Description.PublishInfo.Series) > 0 {
		return &model.Serie{Name: parsers.Title(fb.Description.PublishInfo.Series[0].Name, fb.Description.TitleInfo.Lang)}
	} else if len(fb.Description.TitleInfo.Series) > 0 {
		return &model.Serie{Name: parsers.Title(fb.Description.TitleInfo.Series[0].Name, fb.Description.TitleInfo.Lang)}
	} else {
		return &model.Serie{}
	}
}
